...
const { COLLECTION_SAILS_USER_DETAIL } = require("../constants/collection.constants");
const { SUCCESS, ERROR, NOT_VALID, USER_TYPE_MARKETING, LOGIN_TYPE_CUSTOM_USER, ACTIVE, DEACTIVE, NO_VALUE, ACCOUNT_VERIFIED } = require("../constants/common.constants");
var uuid = require('uuid/v4');
var { encryptData } = require("../controller/response.controller");

const sails_user_detail_collection = mongoose.model(COLLECTION_SAILS_USER_DETAIL);

/** Sort insert sails user input data */
const sort_insert_sails_user_input_data_controller = async (data) => {
...
/** Insert sails user detail */
const insert_sails_user_detail_controller = async (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            const res = await sails_user_detail_collection.create(data);
...
const get_sails_user_detail_controller = async (query = null, options = { rejectResponse: { status: NO_VALUE, response: "No user found" } }) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!query) {
            ...
            const user = await sails_user_detail_collection.findOne(query, { created_at: 0, updated_at: 0, _id: 0 });
